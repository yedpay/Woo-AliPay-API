 ...
 */
class WoocommerceAlipay extends WC_Payment_Gateway
{
    public function __construct()
    {
        ...
        // Woocommerce Setting
        $this->id = 'alipayapi';
        $this->method_title = __('AliPay API', 'tech');
        $this->method_description = __('Extends WooCommerce to Process Payments with AliPay\'s API Method.', 'tech');
        $this->icon = $this->get_image_path() . 'logo.gif';
        ...
        $this->mode = $this->settings['alipayapi_working_mode'];
        if ($this->mode == 'test') {
            $this->title = $this->settings['alipayapi_title'] . ' - <b>Test Mode</b>';
        } else {
            $this->title = $this->settings['alipayapi_title'];
        }

        $this->description = $this->settings['alipayapi_description'];
        $this->alipayapi_token = $this->settings['alipayapi_token'];
        $this->alipayapi_store_id = $this->settings['alipayapi_store_id'];
        ...
        $this->notify_url = add_query_arg('wc-api', 'woocommercealipay', home_url('/'));
        add_action('woocommerce_api_woocommercealipay', [&$this, 'notify_handler']);
    }
    ...
    public function init_form_fields()
    {
        $this->form_fields = [
            'enabled' => [
                ...
                'type' => 'checkbox',
                'label' => __('Enable AliPay API Payment Module.', 'tech'),
                'default' => 'no'
            ],
            'alipayapi_title' => [
                'title' => __('Title', 'tech'),
    ...
    {
        echo '<h3>' . __('AliPay API Payment Method Configuration', 'tech') . '</h3>';
        echo '<p>' . __('AliPay is the most popular payment gateway for online payment processing') . '</p>';
    ...
    /**
     *  There are no payment fields for AliPay, but want to show the description if set.
     */
    public function payment_fields()
    {
        if ($this->description) {
            echo wpautop(wptexturize($this->description)) . '<br>Please Select the Wallet:<br><br>';
        }
        echo '<select name="alipaywallet">
                <option value="HK" id="alipaywallethk" selected>Alipay HK</option>
                <option value="CN" id="alipaywalletcn">Alipay</option>
            </select>';
    ...
    public function thankyou_page($order_id)
    {
        ...
        $order = new WC_Order($order_id);
        update_post_meta($order_id, 'alipay_order_id', $order_id);
        update_post_meta($order_id, 'alipay_payment_status', isset($_REQUEST['status']) && trim($_REQUEST['status']) != '' ? sanitize_text_field($_REQUEST['status']) : '');
        if (isset($_REQUEST['status']) && sanitize_text_field($_REQUEST['status']) == 'paid' && isset($_REQUEST['key']) && trim($_REQUEST['key']) != '') {
            ...
            $orderNote = 'AliPay API payment completed.<br>
                        AliPay Transaction ID: ' . $transaction_id . '<br>
                        AliPay Order Id: ' . $order_id;

            // updating extra information in databaes corresponding to placed order.
            update_post_meta($order_id, 'alipay_transaction_id', $transaction_id);
            ...
            $orderNote = 'AliPay API payment failed.';
            $order->add_order_note(__($orderNote, 'woocommerce'));
        }
    }
    ...
    {
        echo '<p>' . __('Thank you for your order, please click the button below to pay with AliPay.', 'tech') . '</p>';
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $order = new WC_Order($order_id);
        $wallet = strtoupper($this->get_post('alipaywallet'));
        $extra = [
            'order_id' => $order_id
        ];
        ...
        try {
            $client = new Client($this->operation_mode(), $this->alipayapi_token);
            $client
                ->setCurrency($this->get_currency($currency))
                ->setWallet($this->get_wallet($wallet))
                ->setGateway(4)
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        try {
            ...
            // No response or unexpected response
            $message = "AliPay Refund failed. Couldn't connect to gateway server.";
            $order->add_order_note(__($message, 'woocommerce'));
        ...
        $message = 'AliPay Refund failed, please contact Yedpay.';
        $order->add_order_note(__($message, 'woocommerce'));
        return new WP_Error('Error', $message);
    }
    ...
    {
        return  'AliPay API Refund Completed.<br>
